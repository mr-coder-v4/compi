...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -preset ultrafast -pix_fmt yuv420p(progressive) -vcodec libx265 -vf scale=854:480:flags=lanczos -acodec copy -b:a 320k -c:s copy -map 0 """{out}""" -y'
    elif ffmpeg_cmd == 2:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -crf 22 -preset ultrafast -s 640x360 -c:a copy -b:a 320k -c:s copy -map 0 """{out}""" -y'
    elif ffmpeg_cmd == 3:
        ...
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -preset ultrafast -vcodec libx265 -pix_fmt yuv420p(progressive) -vf scale=1280x720:flags=lanczos -acodec copy -b:a 320k -c:s copy -map 0 """{out}""" -y'
    try:
