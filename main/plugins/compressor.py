...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        ...
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -preset ultrafast -vcodec libx265 -vf scale=1280x720:flags=lanczos -acodec copy -c:s copy -map 0 """{out}""" -y'
    elif ffmpeg_cmd == 6:
