...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        ...
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i "{name}" -threads 15 -preset ultrafast -vcodec libx265 -x265-params "level=6.1:profile=main:high-tier=1:tag=hvc1" -r 30 -crf 25 -color_primaries 1 -color_trc 1 -colorspace 1 -pix_fmt yuv420p -color_range 2 -acodec copy -c:s copy -map 0 "{out}" -y'
    elif ffmpeg_cmd == 3:
