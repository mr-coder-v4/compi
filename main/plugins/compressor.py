...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -map 0 -preset ultrafast -vcodec libx265 -vf scale=854:480 -vf format=yuv420p10le -acodec copy -c:s copy """{out}""" -y'
    elif ffmpeg_cmd == 2:
