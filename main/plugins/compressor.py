...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        ...
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}"""  -preset ultrafast -pix_fmt yuv420p -vcodec libx265 -x265-params profile=main10 -vf scale=1280:720 -acodec copy -c:s copy """{out}""" -y'
    elif ffmpeg_cmd == 6:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}"""  -preset ultrafast -pix_fmt yuv420p10le -vcodec libx265 -x265-params profile=main10 -vf scale=1920:1080 -acodec copy -c:s copy """{out}""" -y'
    try:
