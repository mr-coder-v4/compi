...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -preset ultrafast -vcodec libx265 -pix_fmt yuv420p -vf scale=854:480 -acodec copy -c:s copy """{out}""" -y'
    elif ffmpeg_cmd == 2:
